...
int panel_trigger_get_wpinfo(struct drm_panel *panel, char *buf, size_t size)
{
	...
	for (i = 0; i < 3; i++) {
		read_wp_info[i] = *(unsigned char *)(cmd_msg->rx_buf[0] + i);
...
int panel_get_wp_info(struct drm_panel *panel, char *buf, size_t size)
{
	...
	if (sscanf(led_wp_info_str, "0x%02hhx, 0x%02hhx, 0x%02hhx",
		...
		pr_debug("cmdline parse sucess,read maxlum & wp: %02hhx, %02hhx, %02hhx\n",
			read_wp_info[0], read_wp_info[1], read_wp_info[2]);
	} else {
		...
		ret = snprintf(buf, size, "%02hhx%02hhx%02hhx\n",
			read_wp_info[0], read_wp_info[1], read_wp_info[2]);
		pr_debug("cmdline parse fail,read maxlum & wp: %02hhx, %02hhx, %02hhx\n",
			read_wp_info[0], read_wp_info[1], read_wp_info[2]);
	}
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	pr_debug("l16s_36_02_0a_dsc_vdo %s+\n", __func__);
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "l16s_36_02_0a_dsc_vdo,lcm", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "l16s_36_02_0a_dsc_vdo,lcm",
		.owner = THIS_MODULE,
...
MODULE_PARM_DESC(WpAndMaxlum, "panel-l16s-36-02-0a-dsc-vdo.WpAndMaxlum=<WpAndMaxlum> while <WpAndMaxlum> is 'WpAndMaxlum' ");
module_mipi_dsi_driver(lcm_driver);

MODULE_AUTHOR("Liu Jing <liujing30@xiaomi.com>");
MODULE_DESCRIPTION("L16s 36 02 0a dsc vdo lcd panel driver");
